...
from src.business_logic.process_query import NUM_LAGS
...
def create_splitter(col_label: str) -> Callable[[pd.core.frame.DataFrame],Tuple[pd.core.frame.DataFrame, pd.core.frame.Series]]
...
def create_preprocess_pipeline_train():
    preprocess_pipeline_train = create_pipeline([create_data_fetcher(NUM_LAGS), create_lag_creator(NUM_LAGS, "close"),
                                                 add_label_buy_close, remove_nans,
...
def create_preprocess_pipeline_predict():
    preprocess_pipeline_predict = create_pipeline([create_data_fetcher(NUM_LAGS, last=True), create_lag_creator(NUM_LAGS, "close"),
